...
df1 = lakeDAO.readStreamFrom("hl7_redacted_ok_silver")
...
def transformAndSendToRoute(batchDF, batchId):
    ...
    for program_route in routes_list:
        ...
        goldLakeDAO.writeTableTo(df_one_route, f"{normalize(program_route)}_hl7_redacted_ok_gold")
        # working through each batch of route
...
df1.writeStream.trigger(availableNow=True).option("mergeSchema", "true") \
    .option("checkpointLocation", globalLakeConfig.getCheckpointLocation("hl7_redacted_ok_silver2gold_checkpoint")) \
